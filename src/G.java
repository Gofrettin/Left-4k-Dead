...
/**
 * Updated to work as a desktop application by Toby Davis
 * I added some comments to document the extent of my changes
 */
...
//added JFrame import to take the place of the Applet
import javax.swing.JFrame;


public class G extends Canvas implements Runnable //swapped out Applet for Canvas in order to allow execution as an application
{
...
//                        dir = d * Math.PI * 2 / 64;
        ...
        restart: while (true)
        {
            ...
            winLevel: while (true)
            {
                ...
                while (true)
                {
                    ...
                    if (!running) return; //not 100% sure what this does, but the original !isRunning() check had to be replaced because it did not work with Canvas. the game also seemed to run with this commented out
    ...
    public static void main(String[] args) { //added main so the game can be run as an application
		G game = new G(); //creates game object that will be added to the frame

		JFrame frame = new JFrame("Left 4k Dead"); //defines the title
		frame.add(game, BorderLayout.CENTER); //adds the game to the frame

		//frame.setUndecorated(true);
		frame.setResizable(true); //although the application doesn't scale, this is enable to prevent a border issue
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.setVisible(true); //make frame visible
		frame.setSize(496,518); //sets frame size so the inner frame size is 480x480
